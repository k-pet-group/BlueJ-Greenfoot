...
public class ActorDelegate
{
    ...
    // The object that delegates work to this object
     ...
     * 
     */
    public ActorDelegate(Actor actor)
    {
        this.delegator = actor;
    ...
    private GreenfootImage getClassImage()
    {
        ...
        while (clazz != null) {
            GreenfootImage image = delegator.getImage(clazz);
            if (image != null) {
                return image;
            }
    ...
    private int toCellFloor(int i)
    {
        World aWorld = world;
        if(aWorld == null) {
            aWorld = delegator.getActiveWorld();
        }
        if(aWorld == null) {
            // Should never happen
    ...
    private void sizeChanged()
    {
        ...
        if(world != null) {
            world.updateObjectSize(delegator);
        }
    }   
    ...
    private void locationChanged(int oldX, int oldY)
    {
        ...
        if(world != null) {
            world.updateObjectLocation(delegator, oldX, oldY);
        }
    }
    ...
    {
        failIfNotInWorld();
        return getWorld().getNeighbours(delegator, distance, diagonal, cls);
    }
    ...
    {
        failIfNotInWorld();
        return world.getOneObjectAt(delegator, getX() + dx, getY() + dy, cls);        
    }
    ...
    {
        failIfNotInWorld();
        List l = world.getIntersectingObjects(delegator, cls);
        l.remove(this);
        return l;
    }
    ...
    {
        failIfNotInWorld();
        return world.getOneIntersectingObject(delegator, cls);
    }
...
//    }
    ...
    /**
     * Prepares an object to be inserted into the world at the given coordinate.
     */
