...
/**
  ...
  *
  * @version $Id: install.java 36 1999-04-27 04:04:54Z mik $
  *
	...
	implements ActionListener, LayoutManager2, ImageObserver
{
    ...
    static final int X_LOGO = 10;
    static final int Y_LOGO = 10;

    static final int X_TITLE = 180;
    static final int Y_TITLE = 95;

    static final int Y_LINE1 = 150;
    static final int Y_LINE2 = 190;
    static final int Y_BUTTONS = 220;
    static final int Y_TAGLINE = 270;
    static final int Y_COPYRIGHT = 290;
	...
	if (myPanelNum > 0)
	    makeButton(myPanel,"Previous", q, Y_BUTTONS);
	if (lastOne) {
	    makeButton(myPanel,"Install", 2 * q, Y_BUTTONS);
	} else {
	    makeButton(myPanel,"Next", 2 * q, Y_BUTTONS);
	}
	makeButton(myPanel,"Cancel",3 * q, Y_BUTTONS);

	makeFancyText(myPanelNum,(String)getCapsule("title"),X_TITLE,Y_TITLE,1);
	makeFancyText(myPanelNum,myTagLine,-1,Y_TAGLINE,0);
	...
	myFrame = new Frame("BlueJ Installer");
	...
	//
	makeFancyText(myPanelNum,"Directory to install to: ",10,Y_LINE1,0);
	myDirField = new TextField(20);
	myDirField.setText(System.getProperty("user.dir"));
	myPanel.add(myDirField, new Point(200, Y_LINE1-20));

	if (getCapsule("exeName") != null) {
	    makeFancyText(myPanelNum,"Name of Java executable: ",10,Y_LINE2,0);
	    myJavaField = new TextField(20);
	    myJavaField.setText("java");
	    myPanel.add(myJavaField, new Point(200,Y_LINE2-20));
	}
	...
	// Draw the logo
	//off.drawImage(myImage,(mySize.width-myImageWidth)/2,Y_LOGO,this);
	off.drawImage(myImage, X_LOGO, Y_LOGO, this);
