...
public class MonashImageInspector extends bluej.debugger.InspectorPanel
{
    ...
    {
        ArrayReference img;
        if (obj.getObjectReference() instanceof ArrayReference)
        {
            img = ((ArrayReference) obj.getObjectReference());
        }
        else
        {
            com.sun.jdi.Field dataField = obj.getObjectReference().referenceType().fieldByName("data");
            img = (ArrayReference) obj.getObjectReference().getValue(dataField);
        }
