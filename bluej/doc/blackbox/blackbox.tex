...
experiment identifier that is unlikely to clash with any other researchers'
identifier.  The participant identifier is entirely up to you, although we
...
number, as is standard practice.  (Do not ask the participants to store their name, or
university login or similar, in the participant identifier.)

As described in section \ref{sec:views}, you will be able to retrieve
...
want to investigate this information, see section \ref{sec:views} on how to
...
The event links to an entry in the |bench_objects| table, indicating
...
The event links to an entry in the |bench_objects| tables, indicating
...
book-keeping, and can be ignored.
...
To keep participant information (see section \ref{sec:participants}) hidden,
...
\begin{lstlisting}
select p.participant_identifier from (select @experiment:="myexperiment") unused, participant_identifiers_for_experiment p;
\end{lstlisting}
...
\begin{lstlisting}
select s.* from (select @experiment:="myexperiment") unused, sessions_for_experiment s;
\end{lstlisting}

%schema:sessions_for_experiment_participant
...
\begin{lstlisting}
select s.* from (select @experiment:="myexperiment", @participant:="p001") unused, sessions_for_experiment_participant s;
\end{lstlisting}
...
\item Should we coalesce edits that occur next to each other?
\end{itemize}

\chapter{Beta Artificialities}
