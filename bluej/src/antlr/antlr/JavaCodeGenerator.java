...
public class JavaCodeGenerator extends CodeGenerator {
	...
	public  void gen(LexerGrammar g) throws IOException {
		...
		println("import antlr.ScannerException;");
		println("import antlr.Tokenizer;");
		...
		if (grammar.debuggingOutput)
			println("super(new antlr.debug.DebuggingInputBuffer(ib));");
		else
			println("super(ib);");
	...
	public void gen(ParserGrammar g) throws IOException {
		...
		println("import antlr." + grammar.getSuperClass() + ";");
		println("import antlr.Token;");
		println("import antlr.ParserException;");
		...
		println("protected " + grammar.getClassName()+"(Tokenizer lexer, int k) {");
		...
		println("public " + grammar.getClassName()+"(Tokenizer lexer) {");
				...
				if (unpredicted && alt.semPred==null && alt.synPred==null) {
					...
					if ( alt.semPred != null ) {
						...
						if (((grammar instanceof ParserGrammar) || (grammar instanceof LexerGrammar)) && grammar.debuggingOutput)
							e = "("+e+"&& fireSemanticPredicateEvaluated(antlr.debug.SemanticPredicateEvent.PREDICTING,"+addSemPred(charFormatter.escapeString(alt.semPred))+","+alt.semPred+"))";
						else
							e = "("+e+"&&("+alt.semPred +"))";
					}
	...
	protected void genHeader() 
	{
		...
		println(" * ANTLR Version " + ANTLRParser.version + "; 1996-1998");
		println(" */");
	}
	...
	public void genNextToken() {
		...
		if (!hasPublicRules) {
			println("");
			println("public Token nextToken() throws IOException { return new CommonToken(Token.EOF_TYPE, \"\"); }");
			println("");
			return;
		}
		...
		println("");
		println("public Token nextToken() throws IOException {");
		tabs++;
		println("Token _rettoken=null;");
		_println("tryAgain:");
		println("for (;;) {");
		tabs++;
		...
		String errFinish = "if (LA(1)==EOF_CHAR) {_returnToken = makeToken(Token.EOF_TYPE);}";
		errFinish += newline+"\t\t\t\t";
		if ( ((LexerGrammar)grammar).filterMode ) {
			if ( filterRule==null ) {
				...
				errFinish += "else {"+newline+
						...
						"\t\t\t\t\tcatch(ScannerException e) {"+newline+
		...
		if ( ((LexerGrammar)grammar).filterMode ) {
			if ( filterRule==null ) {
				...
				println("catch(ScannerException ee) {");
...
*/
	...
	protected void genSemPred(String pred) {
		...
		if (grammar.debuggingOutput && ((grammar instanceof ParserGrammar) ||
			...
			pred = "fireSemanticPredicateEvaluated(antlr.debug.SemanticPredicateEvent.VALIDATING,"+addSemPred(escapedPred)+","+pred+")";
		println("if (!("+pred+"))");
	...
	protected void genTokenTypes(TokenManager tm) throws IOException {
		...
		setupOutput(tm.getName() + "TokenTypes");
		...
		println("public static final int EOF = " + Token.EOF_TYPE + ";");
		println("public static final int NULL_TREE_LOOKAHEAD = " + Token.NULL_TREE_LOOKAHEAD + ";");
		...
		for (int i = Token.MIN_USER_TYPE; i < v.size(); i++) {
			...
			if (s != null) {
				if ( s.startsWith("\"") ) {
					...
					println("public static final int " + s + " = " + i + ";");
				}
			}
		}
	...
	public String getASTCreateString(Vector v) {
		...
		buf.append("("+labeledElementASTType+")astFactory.make( (new ASTArray(" + v.size() + "))");
		for (int i = 0; i < v.size(); i++) {
...
*/
	...
	private void setupGrammarParameters(Grammar g) {
		if (g instanceof ParserGrammar) {
			...
			throwNoViable = "throw new NoViableAltException(LT(1));";
		}
			...
			throwNoViable = "throw new ScannerException(\"no viable alt for char: \"+(char)LA(1),getLine());";
		}
