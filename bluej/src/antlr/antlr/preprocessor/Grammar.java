...
class Grammar {
	...
	protected boolean specifiedVocabulary=false;	// found tokdef option?
	...
	public void expandInPlace() {
		...
		if ( superG == null ) return;				// error (didn't provide superclass)
		if ( superG.isPredefined() ) return;		// can't expand Lexer, Parser, ...
...
/*
		if ( !specifiedVocabulary ) {
			antlr.Tool.warning("you probably want a tokdef option in the supergrammar");
		}	
*/
	...
	public void inherit(Option o, Grammar superG) {
		...
		if ( overriddenOption==null ) {
			if ( o.getName().equals("tokdef") ) {
				// get rid of leading/trailing " of tokdef RHS
				String rhs = antlr.Tool.stripFrontBack(o.getRHS(), "\"", "\"");

				// make a copy of tokdef file in current directory.
				String originatingGrFileName = o.getEnclosingGrammar().getFileName();
				String path = antlr.Tool.pathToFile(originatingGrFileName);
				String originalTokdefFileName = path+rhs;
				String newTokdefFileName = antlr.Tool.fileMinusPath(rhs);
				try {
					antlr.Tool.copyFile(originalTokdefFileName, newTokdefFileName);
				}
				catch (IOException io) {
					antlr.Tool.toolError("cannot find tokdef file "+originalTokdefFileName);
					return;
				}		

				// need to modify tokdef option so "tokenfile" is converted
				// to simply "tokenfile"; i.e., get rid of path in front
				o.setRHS("\""+newTokdefFileName+"\";");

/*
				if ( rhs.charAt(0)!=System.getProperty("file.separator").charAt(0) ) {
					// does not begin with /, must not be absolute path
					// prefix the file (or relative path) with the path to
					// the associated grammar file
					String originatingGrFileName = o.getEnclosingGrammar().getFileName();
					String path = antlr.Tool.pathToFile(originatingGrFileName);
					if ( path.equals("."+System.getProperty("file.separator")) ) {
						path = "";	// don't bother putting "./" on front
					}	
					o.setRHS("\""+path+rhs);
				}
*/
			}	
			addOption(o);	// copy option into this
		}
	}
...
/*		DO NOT HAVE SUBGRAMMAR INHERIT FROM SUPERGRAMMAR FOR NOW
		String sup="("+superGrammar+")";
		if (	superGrammar.equals("Parser") ||
				superGrammar.equals("Lexer") ||
				superGrammar.equals("TreeParser") ) {
			sup = "";
		}
*/
		String sup = "";
