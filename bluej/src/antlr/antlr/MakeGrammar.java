...
public class MakeGrammar extends DefineGrammarSymbols {
	...
	public void beginTree(int line) throws SemanticException {
		if (!(grammar instanceof TreeWalkerGrammar)) {
			tool.error("Trees only allowed in TreeParser", line);
		...
		for (int i=0; i<lexRules.size(); i++) {
			...
			if (!r.isDefined()) {
				...
				if ( r.access.equals("public") ) {
					RuleRefElement rr =	new RuleRefElement(g, new CommonToken(r.getId()), GrammarElement.AUTO_GEN_NONE);
					//labelElement(rr, new Token("_rettoken"));
					// rr.setIdAssign("_ttype");
					rr.setLabel("_rettoken");
					rr.enclosingRuleName = "nextToken";
					/*
					String aStr = "_token = _rettoken;";
					ActionElement a = new ActionElement(g, new CommonToken(aStr));
					rr.next = a;
					a.next = ruleEnd;
					*/
					rr.next = ruleEnd;
...
/*
		// Create a reference to EOF
		RuleRefElement rr =	new RuleRefElement(g, new CommonToken("EOF"), GrammarElement.AUTO_GEN_NONE);
		rr.setIdAssign("_ttype");
		rr.next = ruleEnd;
		rb.addAlternative(new Alternative(rr));
		// Add a reference to the rule used for the alt
		// r.addReference(rr);
*/
	...
	private AlternativeBlock createOptionalRuleRef(String rule, int line) {
		...
		if ( Character.isUpperCase(rule.charAt(0)) ) { // lexer rule?
			rule = CodeGenerator.lexerRuleName(rule);
		}
		if ( !grammar.isDefined(rule) ) {
			grammar.define(new RuleSymbol(rule));
		}
		
		// Make the rule ref element
		RuleRefElement rref = new RuleRefElement(grammar, rule, line, GrammarElement.AUTO_GEN_NONE);
	...
	public void defineRuleName(Token r, String access, boolean ruleAutoGen, String docComment) throws SemanticException {
		if ( Character.isUpperCase(r.getText().charAt(0)) ) { 
			if (!(grammar instanceof LexerGrammar)) {
				tool.error("Lexical rule defined outside of lexer", r.getLine());
				throw new SemanticException("Lexical rule defined outside of lexer");
			}
		}
	...
	public void endExceptionSpec() {
		...
		if (context().block instanceof RuleBlock)
		{
			...
			if (context().currentAlt().exceptionSpec != null) {
				tool.error("Alternative already has an exception specification", context().block.getLine());
			}
			else {
		...
		if ( label != null ) {
			...
			for (int i = 0; i < ruleBlock.labeledElements.size(); i++) {
				...
				if (l != null && l.equals(label.getText())) {
					tool.error("Label '" + label.getText() + "' has already been defined", label.getLine());
					return;
				}
			}
	...
	public void oneOrMoreSubRule() {
		if (context().block.not) {
			tool.error("'~' cannot be applied to (...)* subrule", context().block.getLine());
		}
	...
	public void refCharLiteral(Token lit, Token label, boolean inverted, int autoGenType, boolean lastInRule) {
		if (!(grammar instanceof LexerGrammar)) {
			tool.error("Character literal only valid in lexer", lit.getLine());
			return;
		}
		...
		if (
			!((LexerGrammar)grammar).caseSensitive && cl.tokenType < 128 && 
			Character.toLowerCase((char)cl.tokenType) != (char)cl.tokenType
		) {
			tool.warning("Character literal must be lowercase when caseSensitive=false", lit.getLine());
		}
	...
	public void refCharRange(Token t1, Token t2, Token label, int autoGenType, boolean lastInRule) {
		...
		if (!((LexerGrammar)grammar).caseSensitive) {
			if (rangeMin < 128 && Character.toLowerCase((char)rangeMin) != (char)rangeMin) {
				tool.warning("Character literal must be lowercase when caseSensitive=false", t1.getLine());
			}
			if (rangeMax < 128 && Character.toLowerCase((char)rangeMax) != (char)rangeMax) {
				tool.warning("Character literal must be lowercase when caseSensitive=false", t2.getLine());
			}
		}
	...
	public void refReturnAction(Token returnAction) {
		if (grammar instanceof LexerGrammar) {
			...
			if (rs.access.equals("public")) {
				tool.warning("public Lexical rules cannot specify return type", returnAction.getLine());
				return;
			}
		}
	...
	public void refRule(Token idAssign, Token r, Token label, Token args, int autoGenType)
	{
		...
		if (grammar instanceof LexerGrammar) {
			...
			if (autoGenType == GrammarElement.AUTO_GEN_CARET) {
				tool.error("AST specification ^ not allowed in lexer", r.getLine());
			}
		}
	...
	public void refStringLiteral(Token lit, Token label, int autoGenType, boolean lastInRule) {
		...
		if (grammar instanceof TreeWalkerGrammar && autoGenType == GrammarElement.AUTO_GEN_CARET) {
			tool.error("^ not allowed in here for tree-walker", lit.getLine());
		}
		...
		if (grammar instanceof LexerGrammar && !((LexerGrammar)grammar).caseSensitive) {
			for (int i = 1; i < lit.getText().length()-1; i++) {
				...
				if (c < 128 && Character.toLowerCase(c) != c) {
					tool.warning("Characters of string literal must be lowercase when caseSensitive=false", lit.getLine());
					break;
				}
			}
		}
	...
	public void refToken(Token idAssign, Token t, Token label, Token args,
		...
		if (grammar instanceof LexerGrammar) {
			...
			/*
			if (label != null) {
				tool.warning("label on token reference in lexer was ignored", t.getLine());
			}
			*/
			if (autoGenType == GrammarElement.AUTO_GEN_CARET) {
				tool.error("AST specification ^ not allowed in lexer", t.getLine());
			}
			if (inverted) {
				tool.error("~TOKEN is not allowed in lexer", t.getLine());
			}
			...
			if (idAssign!= null)
			{
				tool.error("Assignment from token reference only allowed in lexer", idAssign.getLine());
			}
			if (args != null)
			{
				tool.error("Token reference arguments only allowed in lexer", args.getLine());
			}
	...
	public void refTokenRange(Token t1, Token t2, Token label, int autoGenType, boolean lastInRule) {
		if (grammar instanceof LexerGrammar) {
			tool.error("Token range not allowed in lexer", t1.getLine());
			return;
		}
	...
	public void synPred() {
		if (context().block.not) {
			tool.error("'~' cannot be applied to syntactc predicate", context().block.getLine());
		}
		// create the right kind of object now that we know what that is
		// and switch the list of alternatives.  Adjust the stack of blocks.
		// copy any init action also.
