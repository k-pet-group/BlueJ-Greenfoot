...
/**
 * A class reperesenting a TextArea.
 *
 * Created: Oct 1, 1998.
 *
 ...
 * @version 1.0
 */
class GUITextArea extends TextArea implements GUIComponentLeaf
{
    ...
    /**
     * Constructs a GUITextArea with no text.
     *
     ...
     */
    public GUITextArea(GUIComponentNode parent, StructureContainer structCont, GUIBuilderApp app)
    {
	super();
	this.parent = parent;
	this.structCont = structCont;
	this.app = app;
	setName("textArea"+counter);
	counter++;
    }


    /**
     * Constructs a GUITextArea with text text.
     *
     * @param parent	    The component's parent in the tree structure.
     * @param structCont    The StructureContainer containing the component.
     * @param app	    The main application.
     */
    public GUITextArea(String text, GUIComponentNode parent, StructureContainer structCont, GUIBuilderApp app)
    {
	super(text);
	this.parent = parent;
	this.structCont = structCont;
    ...
    /**
     * Constructs a GUITextArea with text text and rows Rows and cols Columns.
     *
     * @param parent	    The component's parent in the tree structure.
     * @param structCont    The StructureContainer containing the component.
     * @param app	    The main application.
     */
    public GUITextArea(String text, int rows, int cols, int scrollbars, GUIComponentNode parent, StructureContainer structCont, GUIBuilderApp app)
    {
	...
	StringBuffer initCode = new StringBuffer ("new TextArea (");
	if (!(getText().equals("")))
	    initCode.append ("\""+getText()+"\"");
        if(defined || getScrollbarVisibility()!=TextArea.SCROLLBARS_BOTH){
            initCode.append(","+getRows()+","+getColumns());
            if(getScrollbarVisibility()!=TextArea.SCROLLBARS_BOTH)
                initCode.append(","+getScrollbarVisibilityAsText());
        }
	...
	{
	    String variableCode = new String("TextArea "+getName()+" = ");
    ...
    {
	TextArea preview = new TextArea(getText(), getRows(), getColumns(), getScrollbarVisibility());
    ...
    {
        GUITextAreaPropertyDialog propertyDialog = new GUITextAreaPropertyDialog(app,this,"TextArea",structCont);
    ...
    private String getScrollbarVisibilityAsText()
        {
            if(getScrollbarVisibility() == TextArea.SCROLLBARS_VERTICAL_ONLY)
                return "TextArea.SCROLLBARS_VERTICAL_ONLY";
            if(getScrollbarVisibility() == TextArea.SCROLLBARS_HORIZONTAL_ONLY)
                return "TextArea.SCROLLBARS_HORIZONTAL_ONLY";
            if(getScrollbarVisibility() == TextArea.SCROLLBARS_NONE)
                return "TextArea.SCROLLBARS_NONE";
            else
                return "";
