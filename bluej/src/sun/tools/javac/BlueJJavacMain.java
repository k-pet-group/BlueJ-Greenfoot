...
public
class BlueJJavacMain implements Constants {
    /**
     * Name of the program.
     */
    String program;
...
//    }
// BLUEJ
    public BlueJJavacMain(OutputStream out) {
	this.out = out;
	this.program = "javac-bluej";
    }
// end BLUEJ
    /**
     * Exit status.
    ...
    private static final String[] releases =      { "1.1", "1.2" };
    private static final short[] majorVersions =  {    45,    46 };
    private static final short[] minorVersions =  {     3,     0 };

    /**
     * Run the compiler
     */
    public synchronized boolean compile(String argv[], JavacWatcher watcher) {
...
// end JCOV
	...
	try {
		...
		if (c.getStatus() == CS_PARSED) {
			...
			continue;
		    try {
// REMOVED FOR BLUEJ	c.getClassDefinition(env);
// BLUEJ
			ClassDefinition defn = c.getClassDefinition(env);
			if((watcher != null)
                           && c.isDefined()
                           && !defn.isSynthetic()
                           && (defn instanceof SourceClass))
                            watcher.notifyParsed(c, (SourceClass)defn, env);
// end BLUEJ
