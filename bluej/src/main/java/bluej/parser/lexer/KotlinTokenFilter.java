...
/**
 * This is a token stream processor for Kotlin code. It removes whitespace and comment tokens from the
 * stream, attaching comments as hidden tokens to the next suitable token.
 */
public final class KotlinTokenFilter implements TokenStream
{
    ...
    public KotlinTokenFilter(TokenStream source)
    {
        sourceStream = source;
        lastComment = null;
    }

    public KotlinTokenFilter(TokenStream source, KotlinParser parser)
    {
        this(source);
        this.parser = parser;
    }
    ...
    private LocatableToken nextToken2()
    {
        ...
        while (true) {
            ...
            int ttype = t.getType();
            if (ttype == KotlinTokenTypes.ML_COMMENT) {
            ...
            else if (ttype == KotlinTokenTypes.SL_COMMENT) {
                if (parser != null) {
                    parser.gotComment(t);
                }
            }
