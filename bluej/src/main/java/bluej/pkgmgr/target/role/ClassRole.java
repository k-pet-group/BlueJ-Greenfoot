/*
 ...
 Copyright (C) 1999-2009,2011,2014,2016,2017,2019,2020,2023,2025,2026  Michael Kolling and John Rosenberg
...
/**
 * A class role in a class target (which may be a Java, Stride, or Kotlin class),
 * providing behaviour specific to particular class types. Note that Kotlin
 * function files use {@link KotlinFileRole} instead.
 *
 * @author Bruce Quig
 */
public abstract class ClassRole
{
     ...
     */
    public static boolean generateSkeleton(String template, Package pkg, String name, String sourceFile, boolean includeFullContent)
    {
        ...
        if (pkg.isUnnamedPackage()) {
            ...
            String terminator = sourceFile.toLowerCase().endsWith(".kt") ? "" : ";";
        ...
        for (int i = first; i < last; i++) {
            try {
                ...
                if (m instanceof MethodView) {
                    MethodView mv = (MethodView) m;
                    String desc = prefix + mv.getLongDesc() + mv.getAutoGeneratedSuffix();
                    ops.add(new ExecuteMethodAction(desc, il, mv));
                }
