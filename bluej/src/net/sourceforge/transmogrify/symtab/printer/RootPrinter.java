...
// $Id: RootPrinter.java 1014 2001-11-30 03:28:10Z ajp $

/**
 * skips the current node, but walks each of the children
 */

public class RootPrinter extends DefaultPrinter {
  public RootPrinter(SymTabAST nodeToPrint) {
    super(nodeToPrint);
  }
  ...
  public void printSelf(PrettyPrinter printout) throws IOException {
    ...
    while (child != null) {
      ...
      if (child.getType() == JavaTokenTypes.IMPORT) {
        while (child.getNextSibling().getType() == JavaTokenTypes.IMPORT) {
