...
// $Id: TableMaker.java 1011 2001-11-22 10:36:26Z ajp $
...
public class TableMaker {
  ...
  public SymbolTable getTable() throws SymbolTableException {
    if (symbolTable == null) {
      ...
      System.out.println("resolution took " +
                         (end-start) / 1000.0 +
                         " seconds");

      symbolTable.update(System.currentTimeMillis());
    }
  ...
  private void createDefinitions() throws SymbolTableException {
    ...
    System.out.println("initial walk took " +
                       (end-start) / 1000.0 +
                       " seconds");
    ...
    System.out.println("definition finishing took " +
                       (end-start) / 1000.0 +
                       " seconds");
  }
  ...
  private void finishCreatingDefinition(Definition def) throws SymbolTableException {
    ...
    if (def instanceof AnonymousInnerClass) {
      ...
      if (firstChild.getType() == JavaTokenTypes.PARAMETER_DEF) {
  ...
  public void walkTree(SymTabAST tree, boolean makeAnonymousScopes) {
    ...
    if (tree != null) {
      ...
      // REDTAG -- this needs to be delegated away from symbol table
      int line = ASTUtil.getLine(tree);
      if ( line != 0) {
        symbolTable.setCurrentLine(line);
      }

      switch(tree.getType()) {
      case 0:
        processFile(tree);
        if ( tree.getFirstChild().getType() != JavaTokenTypes.PACKAGE_DEF ) {
      ...
      case JavaTokenTypes.LITERAL_new:
        ...
        SymTabAST objblock
          = symtabTree.getFirstChildOfType(JavaTokenTypes.OBJBLOCK);
        if (objblock != null) {
          SymTabAST classExtended
            = symtabTree.getFirstChildOfType(JavaTokenTypes.IDENT);
  ...
  public void processClass(SymTabAST tree) {
    ...
    makeClass(name, tree);
    walkSiblings((SymTabAST)tree.getFirstChildOfType(JavaTokenTypes.OBJBLOCK).getFirstChild(),
                 false);
    symbolTable.popScope();
  }
  ...
  private void processVariableDef(SymTabAST tree) {
    ...
    SymTabAST assignmentNode
      = ((SymTabAST)tree).getFirstChildOfType(JavaTokenTypes.ASSIGN);
  ...
  public void processTry(SymTabAST tree){
    ...
    SymTabAST slist = tree.getFirstChildOfType(JavaTokenTypes.SLIST);
  ...
  public void processFinally(SymTabAST tree){
    ...
    SymTabAST slist = tree.getFirstChildOfType(JavaTokenTypes.SLIST);
    SymTabAST tryBlock = tree.getFirstChildOfType(JavaTokenTypes.LITERAL_try);
  ...
  public void processMethodDef(SymTabAST tree) {
    ...
    walkTree(tree.getFirstChildOfType(JavaTokenTypes.SLIST), false);
    symbolTable.popScope();
  }
  ...
  public void processFor(SymTabAST tree) {
    ...
    symbolTable.pushScope( block );
    walkTree(tree.getFirstChildOfType(JavaTokenTypes.FOR_INIT), false);
    walkTree(tree.getFirstChildOfType(JavaTokenTypes.FOR_CONDITION), false);

    SymTabAST forIter = tree.getFirstChildOfType(JavaTokenTypes.FOR_ITERATOR);
  ...
  public void processIf(SymTabAST tree) {
    ...
    SymTabAST expr = tree.getFirstChildOfType(JavaTokenTypes.EXPR);
  ...
  public void processElse(SymTabAST tree) {
    if (tree != null) {
      if (tree.getType() == JavaTokenTypes.LITERAL_if) {
        processIf(tree);
      }
      else {
        makeElseBlock(tree);
  ...
  public void makeElseBlock(SymTabAST tree) {
    if (tree.getType() == JavaTokenTypes.SLIST) {
      BlockDef block = makeBlock((SymTabAST)tree);
      symbolTable.pushScope( block );
      walkTree(tree, false);
      symbolTable.popScope();
  ...
  public void processBlock(SymTabAST tree, boolean makeAnonymousScopes) {
    BlockDef block = makeBlock((SymTabAST)tree);
    symbolTable.pushScope(block);
    walkSiblings((SymTabAST)tree.getFirstChild(), makeAnonymousScopes);
    symbolTable.popScope();
  }
  ...
  public static SymTabAST getExtendsNode(ClassDef classDef) {
    ...
    extendsClause =
      classDefTreeNode.getFirstChildOfType(JavaTokenTypes.EXTENDS_CLAUSE);
  ...
  class DefinitionFinisher {
    ...
    */
     ...
     * @param typeNode the JavaTokenTypes.TYPE node associated with the def
     *
  ...
  class ClassFinisher extends DefinitionFinisher {
    ...
    */
    private void addImports() throws ClassImportException {
      ...
      for ( int i = 0; i < unprocessedImports.size(); i++ ) {
        ...
        if (lastPart.getType() == JavaTokenTypes.IDENT) {
    ...
    */
    private void setSuperclass() {
      if (_def.getTreeNode().getType() == JavaTokenTypes.CLASS_DEF) {
    ...
    */
    private void addInterfaces() {
      ...
      if (_def.getTreeNode().getType() == JavaTokenTypes.CLASS_DEF) {
        implementsClause
          = _node.getFirstChildOfType(JavaTokenTypes.IMPLEMENTS_CLAUSE);
      }
      else {
        implementsClause
          = _node.getFirstChildOfType(JavaTokenTypes.EXTENDS_CLAUSE);
      }
  ...
  class VariableFinisher extends DefinitionFinisher {
    ...
    */
    public void finish() {
      ...
      SymTabAST typeNode = _node.getFirstChildOfType(JavaTokenTypes.TYPE);

      SymTabAST typeTextNode = (SymTabAST)(typeNode.getFirstChild());
      if ( typeTextNode.getType() == JavaTokenTypes.ARRAY_DECLARATOR ) {
  ...
  class MethodFinisher extends DefinitionFinisher {
    ...
    */
    private void setSignature() {
      SymTabAST parametersNode
        = _node.getFirstChildOfType(JavaTokenTypes.PARAMETERS);
    ...
    */
    private void setExceptionsThrown() {
      ...
      SymTabAST throwsNode
        = _node.getFirstChildOfType(JavaTokenTypes.LITERAL_throws);

      if ( throwsNode != null ) {
        SymTabAST exceptionNode = (SymTabAST)(throwsNode.getFirstChild());
        while (exceptionNode != null ) {
          if (exceptionNode.getType() == JavaTokenTypes.DOT) {
    ...
    */
    private SymTabAST getTypeNode() {
      return _node.getFirstChildOfType(JavaTokenTypes.TYPE);
    }
  }
  ...
  class CatchFinisher extends DefinitionFinisher {
    ...
    private void createExceptionVariable() {
      SymTabAST exceptionNode
        = _def.getTreeNode().getFirstChildOfType(JavaTokenTypes.PARAMETER_DEF);
